          ...
          {data.map((p) => {
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-moss/75">
                      ...
                      <span className="inline-flex items-center gap-1">
                        <MapPin size={12} /> {String(lead["postcode"] ?? "")}{" "}
                        {String(lead["house_number"] ?? "")} {String(lead["city"] ?? "")}
                      </span>
                    </p>
                ...
                {lead["notes"] ? (
                  <p className="mt-3 rounded-xl bg-muted p-3 text-xs text-moss/80">
                    {String(lead["notes"])}
                  </p>
                ) : null}
                  ...
                  {team.data?.canAssign && (
                    <label className="ml-auto flex items-center gap-2 text-xs text-moss/70">
                      ...
                      <select
                        value={p.assignedTo ?? ""}
                        onChange={(e) =>
                          assign.mutate({ purchaseId: p.id, assignedTo: e.target.value || null })
                        }
